            ...
            <div>
                ...
                @foreach ($machinetypes as $machinetype)
                    <div class="flex items-center">
                        ...
                        <input type="number" name="buy[{{ $machinetype->id }}]" id="buy[{{ $machinetype->id }}]" min="0"
                ...
                @foreach ($machines as $machine)
                    <div class="flex items-center mb-2">
                        <input type="checkbox" name="sell[]" id="sell_{{ $machine->id }}" value="{{ $machine->id }}"
                            class="mr-2">

                        <label for="sell_{{ $machine->id }}" class="text-gray-800 dark:text-gray-200">
                            {{ trans_choice('messages.machineType', 1) }}: {{ $machine->machinetype_id }}, ID:
                            {{ $machine->id }}
                        </label>
                    </div>
                @endforeach
                ...
                <input type="checkbox" name="approve" id="approve" required
                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded checked:bg-red-500">
            </div>
            <div>
                {{-- <button type="submit"
